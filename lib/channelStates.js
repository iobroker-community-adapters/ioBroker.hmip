...
const CHANNEL_STATES = {
    ...
    CARBON_DIOXIDE_SENSOR_CHANNEL: {
        states: {
            ...
            vaporAmount: { type: 'number', role: 'value' },
        },
    },
    ...
    GENERIC_CLIMATE_SENSOR_CHANNEL: {
        states: {
            ...
            todayRainCounter: { type: 'number', role: 'value', unit: 'mm', derive: 'millimetres' },
            todaySunshineDuration: { type: 'number', role: 'value', unit: 'min' },
            totalRainCounter: { type: 'number', role: 'value', unit: 'mm', derive: 'millimetres' },
            ...
            yesterdayRainCounter: { type: 'number', role: 'value', unit: 'mm', derive: 'millimetres' },
    ...
    MULTI_MODE_INPUT_BLIND_CHANNEL: {
        states: {
            ...
            delayCompensationValue: { type: 'number', role: 'value.interval' },
            endpositionAutoDetectionEnabled: { type: 'boolean', role: 'indicator' },
            favoritePrimaryShadingPosition: { type: 'number', role: 'value' },
            favoriteSecondaryShadingPosition: { type: 'number', role: 'value' },
    ...
    UNIVERSAL_LIGHT_CHANNEL: {
        states: {
            channelActive: { type: 'boolean', role: 'indicator' },
            colorTemperature: {
                type: 'number',
            ...
            humanCentricLightActive: { type: 'boolean', role: 'indicator' },
    ...
    WEATHER_SENSOR_CHANNEL: {
        states: {
            ...
            illuminationThresholdSunshine: { type: 'number', role: 'value' },
            storm: { type: 'boolean', role: 'indicator' },
            sunshine: { type: 'boolean', role: 'indicator' },
    ...
    WEATHER_SENSOR_PLUS_CHANNEL: {
        states: {
            ...
            storm: { type: 'boolean', role: 'indicator' },
            sunshine: { type: 'boolean', role: 'indicator' },
            todayRainCounter: { type: 'number', role: 'value.rain.today', unit: 'mm', derive: 'millimetres' },
            todaySunshineDuration: { type: 'number', role: 'value' },
            totalRainCounter: { type: 'number', role: 'value', unit: 'mm', derive: 'millimetres' },
            totalSunshineDuration: { type: 'number', role: 'value' },
            vaporAmount: { type: 'number', role: 'value' },
...
function channelStateValues(states, functionalChannel) {
    ...
    for (const field of Object.keys(states)) {
        ...
        if (Object.prototype.hasOwnProperty.call(spec, 'constant')) {
            ...
            // the only deriver that reads the whole channel rather than one field of it
            value = DERIVERS.windowOpen(channel);
        } else if (spec.derive) {
            value = DERIVERS[spec.derive](channel[spec.from ?? field]);
        } else {
